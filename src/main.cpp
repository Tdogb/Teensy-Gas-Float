...
void configure(data *d) {
	...
	float max_abs = d->mc_current_max;//VESC_IF->get_cfg_float(CFG_PARAM_l_abs_current_max);
	int mabs = max_abs;
	// d->max_continuous_current = (max_abs - mabs) * 100;
	d->max_continuous_current = 999;
...
void calculate_setpoint_target(data *d) {
	float input_voltage = mc_get_input_voltage_filtered();
			  ...
			  (d->abs_erpm > 2000))								// acceleration can jump a lot at very low speeds
	{
	...
	} else if(mc_temp_fet_filtered() > d->mc_max_temp_fet){
		...
		if(mc_temp_fet_filtered() > (d->mc_max_temp_fet + 1)) {
			if(d->erpm > 0){
	...
	} else if(mc_temp_motor_filtered() > d->mc_max_temp_mot){
		// Use the angle from Low-Voltage tiltback, but slower speed from High-Voltage tiltback
		beep_alert(d, 3, true);	// Triple-beep (long beeps)
		d->beep_reason = BEEP_TEMPMOT;
		if(mc_temp_motor_filtered() > (d->mc_max_temp_mot + 1)) {
			if(d->erpm > 0){
				d->setpoint_target = d->float_conf.tiltback_lv_angle;
			} else {
				d->setpoint_target = -d->float_conf.tiltback_lv_angle;
...
void float_thd(void *arg) {
	...
	while (!should_terminate()) {
		...
		d->true_pitch_angle = RAD2DEG_f(ahrs_get_pitch(&d->m_att_ref));
		d->pitch_angle = RAD2DEG_f(imu_get_pitch());
		if (d->is_flywheel_mode) {
		...
		d->duty_cycle = mc_get_duty_cycle_now();
		d->abs_duty_cycle = fabsf(d->duty_cycle);
		d->erpm = mc_get_rpm();
		d->abs_erpm = fabsf(d->erpm);
		d->adc1 = analogRead(PIN_ADC1);//VESC_IF->io_read_analog(VESC_PIN_ADC1);
		d->adc2 = analogRead(PIN_ADC2);//VESC_IF->io_read_analog(VESC_PIN_ADC2); // Returns -1.0 if the pin is missing on the hardware
		if (d->adc2 < 0.0) {
			d->adc2 = 0.0;
		}
		...
		// Turn Tilt:
		d->yaw_angle = ahrs_get_yaw(&d->m_att_ref) * 180.0f / M_PI;
		...
		d->switch_state = check_adcs(d); // ADCs are measured later in float_thd
		...
		case (STARTUP):
				...
				if (imu_startup_done()) {
					...
					float bat_volts = mc_get_input_voltage_filtered();
...
//		app_balance_experiment();
		...
		// Delay between loops
		delayMicroseconds((uint32_t)((d->loop_time_seconds - roundf(d->filtered_loop_overshoot)) * 1000000.0));
	}
}
